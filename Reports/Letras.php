...
class EnLetras
{
   ...
   * @return string
   */
  public function ValorEnLetras($number, $decimals = 2, $currency = '', $cents = '')
  {
    $this->checkApocope();
  ...
  {
    return $this->ValorEnLetras($number, $decimals, $whole_str, $decimal_str);
  }
  ...
  {
    return implode(
      ' ' . mb_strtoupper($this->conector, 'UTF-8') . ' ',
      array_filter($splitNumber)
    );
  }
