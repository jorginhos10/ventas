...
class Person
{
	...
	public function mostrarCompras(string $idpersona)
	{
		$sql = "SELECT IFNULL(count(*),0) AS cantidadCompras FROM ingreso WHERE idproveedor=?";
		$arrData = array($idpersona);
		return  $this->conexion->getData($sql, $arrData);
	}
	public function mostrarVentas(string $idpersona)
	{
		$sql = "SELECT IFNULL(count(*),0) AS cantidadVentas FROM venta WHERE idcliente=?";
		$arrData = array($idpersona);
		return  $this->conexion->getData($sql, $arrData);
	}
